...
CLASS lhc_Sales IMPLEMENTATION.
  ...
  METHOD create.
    ...
    LOOP AT entities ASSIGNING FIELD-SYMBOL(<lf_sales>).
      ...
      DATA(lo_instance) = zcl_rpz_sales=>get_instance(  ).
      DATA(ls_sales) = CORRESPONDING ztrpz_sales_un( <lf_sales> ).

      ls_sales-orderid = lo_uuid->create_uuid_x16(  ).
      APPEND ls_sales TO zcl_rpz_sales=>gt_sales.
  ...
  METHOD update.
    ...
    TYPES: tt_sales   TYPE TABLE OF ztrpz_sales_un WITH DEFAULT KEY,
           tt_sales_x TYPE TABLE OF zsrpz_sales_un WITH DEFAULT KEY.
    ...
    IF lt_sales IS NOT INITIAL.
      ...
      zcl_rpz_sales=>gt_sales = VALUE #( FOR ls IN lt_sales
...
CLASS lsc_ZI_RPZ_SALES DEFINITION INHERITING FROM cl_abap_behavior_saver.
PROTECTED SECTION.
...
CLASS lsc_ZI_RPZ_SALES IMPLEMENTATION.
...
METHOD save.
  ...
  MODIFY ztrpz_sales_un FROM TABLE @zcl_rpz_sales=>gt_sales.

  CLEAR zcl_rpz_sales=>gt_sales.
